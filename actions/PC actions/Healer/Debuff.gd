...
func sort_by_distance(target1, target2):
	...
	if target1_distance == target2_distance:
		# if distance is equal, sort by power
		return target1.character_resource.power > target2.character_resource.power
	else:
...
func _physics_process(delta: float) -> void:
	...
	if hit:
		# slow down velocity
		if animatable_body.global_position.distance_to(target.global_position) < 10:
...
func _on_target_area_body_entered(body) -> void:
	...
	if body is Character && body.is_in_group(target_group):
		...
		var target_agility: float = body.character_resource.agility
		var target_defense: float = body.character_resource.defense
